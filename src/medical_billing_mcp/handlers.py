...
def lookup_modifier(
    ...
    if not modifier:
        return {
            "error": "Provide 'modifier' parameter",
    ...
    return {
        "error": f"Modifier '{modifier}' not found",
        "available": list(modifiers.keys())
    }
...
def lookup_payer(
    ...
    if not payer:
        return {
            "error": "Provide 'payer' parameter",
            "available": list(payers.keys())
        }
    ...
    if matches:
        return {
            "error": f"Payer '{payer}' not found exactly",
            "suggestions": matches
        }
    
    return {
        "error": f"Payer '{payer}' not found",
        "available": list(payers.keys())
    }
